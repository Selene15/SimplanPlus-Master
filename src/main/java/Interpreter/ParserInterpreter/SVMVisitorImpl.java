    ...
    public Void visitInstruction(SVMParser.InstructionContext ctx) { 
    	switch (ctx.getStart().getType()) {
			...
			case SVMLexer.BRANCHNEQ:
				...
				labelRef.put(i++,(ctx.l!=null? ctx.l.getText():null));
				break;
			case SVMLexer.MOREEQ:
				code[i++] = SVMParser.MOREEQ;
				break;
			case SVMLexer.LESSEQ:
				code[i++] = SVMParser.LESSEQ;
				break;
			case SVMLexer.MOREOP:
				code[i++] = SVMParser.MOREOP;
				break;
			case SVMLexer.LESSOP:
				code[i++] = SVMParser.LESSOP;
				break;
			case SVMLexer.EQ:
				code[i++] = SVMParser.EQ;
				break;
			case SVMLexer.NEQ:
				...
				break;
			case SVMLexer.BRANCHFUN:
				code[i++] = SVMParser.BRANCHFUN;
				labelRef.put(i++,(ctx.l!=null? ctx.l.getText():null));
				break;
			case SVMLexer.JS:
				code[i++] = SVMParser.JS;
